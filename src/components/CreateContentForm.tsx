...
export function CreateContentForm({ onFinishCreate = () => {} }: CreateContentFormProps) {
    const { username } = useParams() as PageParams

    const { handleSubmit, register, watch, formState: {errors, isSubmitting} } = useForm<CreateContentFormData>({
    ...
    return (
        <div className='rounded-2xl border border-[#999999] p-6'>
          <strong className='font-bold text-[22px] block mb-6'>What&apos; on your mind?</strong>

          <form className='flex flex-col gap-6' onSubmit={handleSubmit(handleCreateContent)}>
            ...
            <div className="flex justify-end">
              <button disabled={isSomeFieldEmpty || isSubmitting} type="submit" className="bg-[#7695EC] uppercase text-white px-8 py-2 items-center justify-center rounded-lg font-bold leading-none focus:outline-none hover:bg-[#4571ec] transition-colors disabled:bg-zinc-500 disabled:cursor-not-allowed">
